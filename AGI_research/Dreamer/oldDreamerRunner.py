...
class DreamerObj:
    ...
    def forward(self,frame,i,state=None):
            obs = {
                    'image':frame,
            ...
            policy_output,state=self.DreamerModel._policy(obs=obs, state=state, training=None)
    ...
    def run(self,trainNum,episodeLength,evalPeriod=10):
        ...
        for episode in range(episodeLength):
            ...
            for i in range(self.config.batch_length):
                ...
                if self.taskID == 1:
                    frame = frame.copy()
                cutAction,actualAction,state=self.forward(frame,i,state)
                next_obs, rewards, terminations, truncations, infos=self.env.step(cutAction.detach().cpu().numpy())
        
                # if i == 0:
                #     # First step: no previous state, but we need to know the structure
                #     # Get structure from current state to create zeros
                #     if state is not None:
                #         stateSave = TensorDict({
                #             k: torch.zeros_like(v).to("cpu") 
                #             for k, v in state[0].items()
                #         }, batch_size=[])
                # elif i > 0 and statePrev is not None:
                #     stateSave = TensorDict({
                #         k: v.to("cpu") 
                #         for k, v in statePrev[0].items()
                #     })
        
                #if i>1:
                    #stateSave=TensorDict({k: v.to("cpu") for k, v in statePrev[0].items()})
    ...
    def train(self,dataDir="/testAGI/AGI_research/Dreamer/dreamerData/",
        ...
        fps = None
        #do we make this apart of the config.dataDir?

        #replace with some random sample

        data=torch.load(fp,weights_only=False)
        print("training",type(data['state']))
        if isinstance(data['state'],torch.Tensor):
            input(prev_state.shape)
