...
def two_hot(x, cfg):
	...
	bin_idx = torch.floor((x - cfg.vmin) / cfg.bin_size).to(torch.long)
	...
	# bin_idx = bin_idx.view(-1, 1).long()          # [batch, 1]
	# bin_offset = bin_offset.view(-1, 1)    # [batch, 1]

	soft_two_hot = soft_two_hot.scatter(1, bin_idx, 1 - bin_offset)
	soft_two_hot = soft_two_hot.scatter(1, (bin_idx + 1) % cfg.num_bins, bin_offset)
	return soft_two_hot
