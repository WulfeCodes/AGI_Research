...
class RunningScale(torch.nn.Module):
	...
	def __init__(self, cfg):
		super().__init__()
		self.cfg = cfg
		self.value = nn.Parameter(torch.ones(1, dtype=torch.float32, device=self.cfg.device))
		self._percentiles = nn.Parameter(torch.tensor([5, 95], dtype=torch.float32, device=self.cfg.device))
	...
	def _percentile(self, x):
		...
		in_sorted=in_sorted.to(self.cfg.device)
		weight_floored = weight_floored.to(self.cfg.device)
		weight_ceiled=weight_ceiled.to(self.cfg.device)
